...
void HariMain(void) {
    ...
    // 右下の枠
    backfill8(vram, 320, COL8_848484, xsize - 47, ysize - 24, xsize - 4, ysize - 24);
    backfill8(vram, 320, COL8_848484, xsize - 47, ysize - 23, xsize - 47, ysize - 4);
    backfill8(vram, 320, COL8_FFFFFF, xsize - 47, ysize - 3, xsize - 4, ysize - 3);
    backfill8(vram, 320, COL8_FFFFFF, xsize - 3, ysize - 24, xsize - 3, ysize - 3);
...
void backfill8(unsigned char *vram, int xsize, unsigned char c, int x0, int y0, int x1, int y1) {
    int x, y;
    for (y = y0; y <= y1; y++) {
        for (x = x0; x <= x1; x++) {
            vram[y * xsize + x] = c;
