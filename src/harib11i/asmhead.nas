...
VBEMODE EQU     0x109           ; 0x105 が元の値だが、不正なモードにして320で続きを行う

		ORG		0xc200			; このプログラムがどこに読み込まれるのか

; VBE存在確認
        MOV     AX,0x9000
        MOV     ES,AX           ; ES:DI の512バイトのアドレスにVBE情報が書き込まれる
        MOV     DI,0
        MOV     AX,0x4f00
        INT     0x10
        CMP     AX,0x004f       ; VBE が存在したらAXが 0x004f になる決まり
        JNE     scrn320
...
; 画面モードを設定
; Makefile の qemu 実行時オプションも合わせて変更
; https://qiita.com/tatsumack/items/491e47c1a7f0d48fc762
		...
		MOV     AX,[ES:DI+0x14]
		MOV     [SCRNY],AX
		;MOV     EAX,[ES:DI+0x28]   ; この2行では動かなかった
		;MOV     [VRAM],EAX
